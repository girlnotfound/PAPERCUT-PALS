...
import { SimpleGrid, Heading, VStack, Box, Input, Button, Text, useToast } from '@chakra-ui/react';
...
const BookOfTheMonth = () => {
  ...
  const { loading: loadingBooks, data: booksData, refetch: refetchBooks } = useQuery(QUERY_BOOKS);
  ...
  const handleToggleFavorite = async (book) => {
    try {
      ...
      await mutation({
        ...
        refetchQueries: [{ query: QUERY_FAVORITEBOOKS, variables: { username: currentUsername } }]
      });
      ...
      toast({
        title: isFavorite ? "Book removed from favorites" : "Book added to favorites",
        status: "success",
        duration: 3000,
        isClosable: true,
      });
  ...
  return (
    <VStack spacing={8} align="stretch">
      <Heading as="h1" size="2xl" textAlign="center">Books of the Month</Heading>
          ...
          <Box key={book._id}>
            ...
            <Box my={4} mx={5} maxWidth={"90%"} display={"flex"} justifyContent={"center"} flexDirection={"column"} alignItems={"center"}>
                ...
                {book.comments.map((comment) => (
                  <Box key={comment._id} p={2} bg="gray.100" borderRadius="md">
                    ...
                    {comment.commentAuthor === currentUsername && (
                      <Button size="xs" onClick={() => handleRemoveComment(book._id, comment._id)}>Remove</Button>
                    )}
                  </Box>
                ))}
              </VStack>
              <Input
                ...
                onChange={(e) => setNewComments(prev => ({ ...prev, [book._id]: e.target.value }))}
